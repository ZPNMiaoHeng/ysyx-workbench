...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  // INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, uint64_t result = ((int64_t)src1 * (int64_t)src2); R(rd) = result >> 32;  printf("%#lx\t%#lx\t%#lx\n", (int64_t)src1, (int64_t)src2, result););
  // INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, int64_t result = ((int64_t)src1 * (int64_t)src2); printf("%#lx\t%#lx\t%#lx\n", (int64_t)src1, (int64_t)src2, result); R(rd) = result >> 32;);  // FIXME:
  // INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) = ((uint64_t)src1 * (uint64_t)src2) >> 32);  // FIXME:
  
  INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or     , R, R(rd) = src1 | src2);
  INSTPAT("0000000 ????? ????? 100 ????? 01100 11", xor    , R, R(rd) = src1 ^ src2);
  INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt    , R, R(rd) = (signed)src1 < (signed)src2); //if((signed)src1 < (signed)src2) R(rd) = 1; else R(rd) = 0);
  ...
  INSTPAT("0000000 ????? ????? 011 ????? 00100 11", sltiu  , I, R(rd) = src1 < imm);
  ...
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, if((signed)src1 < (signed)src2) s->dnpc = s->pc + imm);            //doubt  fuhao
